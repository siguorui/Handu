            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="box">
                        ...
                        <form action="{{url('admin/friendlinks/index')}}">
                        ...
                        <span class="input-group-btn">
                          <button class="btn btn-info btn-flat" type="submit">Go!</button>
                            ...
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                <tr>
                                    <td>{{$v -> id}}</td>
                                    <td class="cname">{{$v -> link_name}}</td>
                                    <td>{{$v -> url}}</td>
                                    <td>@if($v -> status == 1)
                                            开启
                                        @else
                                            <span class="text-red">关闭</span>
                                        @endif</td>
                                    <td style="width: 200px;">
                                    
                                    <a href="{{url('admin/friendlinks/edit')}}/{{$v -> id}}" >编辑 |</a> <a href="{{url('admin/friendlinks/delete')}}/{{$v -> id}}"> 删除</a></td>
                                </tr> 
                                @endforeach
                                </tfoot>
